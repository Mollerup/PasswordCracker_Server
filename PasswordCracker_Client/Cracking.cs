...
namespace PasswordCracker_Client
{
    class Cracking
    {
        public static List<UserInfoClearText> result;
        private readonly HashAlgorithm _messageDigest;
        List<UserInfo> userInfos;
        List<string> wordlistPart;

        public Cracking(List<UserInfo> _userInfos, List<string> _wordlistPart)
        {
            _messageDigest = new SHA1CryptoServiceProvider();
            //_messageDigest = new MD5CryptoServiceProvider();
            // seems to be same speed
            userInfos = _userInfos;
            wordlistPart = _wordlistPart;
        }

        /// <summary>
        /// Runs the password cracking algorithm
        /// </summary>
        public string RunCracking()
        {
            ...
            foreach (string st in wordlistPart)
            {

                IEnumerable<UserInfoClearText> partialResult = CheckWordWithVariations(st, userInfos);
            ...
            string usersFound = null;

            foreach (var v in result)
            {
                usersFound += v.ToString() + "_";
            }
